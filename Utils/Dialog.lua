...
local dialogPool = CreateFramePool("Frame", UIParent, "HironCraftScanDialogTemplate")
local textPool = CreateFramePool("Frame", nil, "HironCraftScanDialogTextTemplate")
local editBoxPool = CreateFramePool("EditBox", nil, "HironCraftScanDialogTextInputTemplate")
local multiLineEditBoxPool = CreateFramePool("ScrollFrame", nil, "HironCraftScanDialogMultiLineTextInputTemplate")
local defaultButtonPool = CreateFramePool("Button", nil, "HironCraftScan_DialogDefaultButtonTemplate")
local checkButtonPool = CreateFramePool("CheckButton", nil, "HironCraftScanDialogCheckButtonTemplate")

HironCraftScan.Dialog = {}

HironCraftScan.Dialog.Element = {
    EditBox = 1,
    Text = 2,
    CheckButton = 3,
...
HironCraftScan_DialogCheckButtonMixin = {}

function HironCraftScan_DialogCheckButtonMixin:OnClick()
...
local function CreateArgs(dialog)
    ...
    for _, frame in ipairs(dialog.frames) do
        if frame.type == HironCraftScan.Dialog.Element.EditBox then
            table.insert(args, FrameToEditBox(frame):GetText());
        end
        if frame.type == HironCraftScan.Dialog.Element.CheckButton then
...
HironCraftScanDialogMixin = {};

function HironCraftScanDialogMixin:CheckEnableSubmit()
...
function HironCraftScanDialogMixin:OnHide()
    if self.initialized then
        ...
        if self.frames then
            for _, frame in ipairs(self.frames) do
                ...
                elseif frame.type == HironCraftScan.Dialog.Element.DefaultButton then
                    defaultButtonPool:Release(frame);
                elseif frame.type == HironCraftScan.Dialog.Element.CheckButton then
        ...
        HironCraftScan.Utils.printTable("Hiding dialog", self.key);
        self.key = nil;
        self.OnAccept = nil;
        self.OnReject = nil;
        self.Validator = nil;
...
HironCraftScanDialogSubmitMixin = {};

function HironCraftScanDialogSubmitMixin:OnClick()
...
HironCraftScan_DialogTextInputAlertIconMixin = {}

function HironCraftScan_DialogTextInputAlertIconMixin:OnEnter()
...
function HironCraftScan_DialogTextInputAlertIconMixin:OnLeave()
    GameTooltip:Hide()
end

HironCraftScanDialogTextInputMixin = {};

function HironCraftScanDialogTextInputMixin:OnLoad()
    self:SetAutoFocus(false);
end

HironCraftScanDialogMultiLineTextInputMixin = {};

function HironCraftScanDialogMultiLineTextInputMixin:OnLoad()
...
HironCraftScan_DialogDefaultButtonMixin = {}

function HironCraftScan_DialogDefaultButtonMixin:OnClick()
    self.EditBox:SetText(self.default_text);
    EditBox_OnEditFocusLost(self.EditBox);
    self:SetEnabled(false);
end

function HironCraftScan_DialogDefaultButtonMixin:Refresh()
...
local function LayoutFramesVertically(frames, parent, padding)
    ...
    for _, frame in ipairs(frames) do
        if frame.type ~= HironCraftScan.Dialog.Element.DefaultButton then
...
function HironCraftScan.Dialog.Show(config)
    ...
    for _, entry in ipairs(config.elements) do
        ...
        local widthAdjust = 0;
        local frame;
        if entry.type == HironCraftScan.Dialog.Element.EditBox then
            ...
            if entry.default_text then
                ...
                defaultButton.type = HironCraftScan.Dialog.Element.DefaultButton;
        ...
        elseif entry.type == HironCraftScan.Dialog.Element.CheckButton then
            ...
            frame.alignLeft = true;
        elseif entry.type == HironCraftScan.Dialog.Element.Text then
        ...
        if entry.type ~= HironCraftScan.Dialog.Element.CheckButton then
    ...
    HironCraftScan.Frames.makeMovable(dialog);
    dialog:SetPoint("CENTER", UIParent, "CENTER", 0, 0)
    dialog.initialized = true;
    dialog.key = config.key;
    uniq[config.key] = dialog;
    HironCraftScan.Utils.printTable("Showing dialog", config.key);
    dialog:Show();
