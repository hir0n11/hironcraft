...
local function CreateBuckets(data, start, end_)
    ...
    for i in ipairs(data) do
        local timestamp = HironCraftScan.Analytics.GetTimeStamp(data[i]);
    ...
    if endIndex == 0 and HironCraftScan.Analytics.GetTimeStamp(data[#data]) <= end_ then
        endIndex = #data;
    end
...
local function PlotDataRange(start, end_)
    ...
    for _, timestamp in pairs(data) do
        local ts = HironCraftScan.Analytics.GetTimeStamp(timestamp);
...
function HironCraftScan.Utils.ShowTooltipPlot(anchor, itemID, data, togglePlotLock)
    ...
    if not plot then
        plot = CreateFrame("GameTooltip", "HironCraftScanTooltipPlot", UIParent, "GameTooltipTemplate");
    ...
    local start = HironCraftScan.Analytics.GetTimeStamp(data[1]);
    local end_ = HironCraftScan.Analytics.GetTimeStamp(data[#data]);
    plot.data = data;
    PlotDataRange(start, end_);
...
function HironCraftScan.Utils.ForceHideTooltipPlot()
    plotLock = nil;
    HironCraftScan.Utils.HideTooltipPlot();
end

function HironCraftScan.Utils.HideTooltipPlot()
