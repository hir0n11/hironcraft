...
HironCraftScan.Debug = {}

function HironCraftScan.Debug.Enable(value)
    DEBUG = value
    print("HironCraftScan debugging" .. (DEBUG and " enabled (use /dev to see output)" or "disabled"))
    if DEBUG then
        HironCraftScan.Debug.Print(HironCraftScan, "HironCraftScan")
    end
end

HironCraftScan.Debug.IsEnabled = function() return DEBUG end

function HironCraftScan.Debug.Print(data, label)
    if not DEBUG then return; end
    if not DevTool then
        local loaded, reason = C_AddOns.LoadAddOn("DevTool");
        if not loaded then
            print("HironCraftScan: Error loading DevTools:", reason)
            return
        end
    end
...
function HironCraftScan.Debug.DrawGrid(frame, spacing)
...
function HironCraftScan.Debug.OutlineFrame(frame)
    if not DEBUG_FRAMES then return end
