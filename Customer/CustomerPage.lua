...
HironCraftScan_FindCrafterButtonMixin = {}

function HironCraftScan_FindCrafterButtonMixin:OnLoad()
...
function DisplayCrafterTable()
    if not foundCrafterPage then
        foundCrafterPage = CreateFrame("Frame", "HironCraftScan_FoundCrafterTable", ProfessionsCustomerOrdersFrame,
            "HironCraftScan_FoundCrafterPageTemplate")
        HironCraftScan.Frames.makeMovable(foundCrafterPage);
        ...
        -- So ESC closes it
        table.insert(UISpecialFrames, "HironCraftScan_FoundCrafterTable")
...
function HironCraftScan.SearchResultsReceived(itemID, player, crafter, greeting, busy, commission)
...
function HironCraftScan_FindCrafterButtonMixin:OnClick()
    foundCrafters = {};
    HironCraftScanComm:FindCrafters(selectedItemID);
    DisplayCrafterTable();
end
...
local function OnCraftingOrderPageOpened()
    if not button then
        button = CreateFrame("Button", "HironCraftScan_FindCrafterButton", ProfessionsCustomerOrdersFrame.Form,
            "HironCraftScan_FindCrafterButtonTemplate");
...
HironCraftScanScannerMenu:RegisterEventCallback("CRAFTINGORDERS_SHOW_CUSTOMER", OnCraftingOrderPageOpened);
HironCraftScanScannerMenu:RegisterEventCallback("CVAR_UPDATE", OnCraftingOrderTypeChanged);
HironCraftScanScannerMenu:RegisterEventCallback("SPELL_DATA_LOAD_RESULT", OnRecipeSelected);

HironCraftScan_FoundCrafterListMixin = {}
...
local function ItsMe(crafter)
    ...
    return HironCraftScan.Utils.ItsMe(crafter, true);
end

function HironCraftScan_FoundCrafterListMixin:Init()
    ...
    local function FrameInitializer(frame, foundCrafter)
        ...
        local crafter = HironCraftScan.NameAndRealmToName(foundCrafter.crafter, true);
        local player = HironCraftScan.NameAndRealmToName(foundCrafter.player, true);
    ...
    view:SetElementFactory(function(factory)
        factory("HironCraftScan_FoundCrafterListElementTemplate", FrameInitializer);
    end);
...
function HironCraftScan_FoundCrafterListMixin:OnHide()
    foundCrafters = nil;
end

HironCraftScan_FoundCrafterListElementMixin = {}

function HironCraftScan_FoundCrafterListElementMixin:OnClick(button)
...
local tooltip = nil;
function HironCraftScan_FoundCrafterListElementMixin:ShowTooltip()
    local name = "HironCraftScan_FoundCrafterTooltip"
    if not tooltip then
    ...
    local greeting = HironCraftScan.Utils.SplitResponse(self.info.greeting);
    ...
    HironCraftScan.Utils.SizeTooltipLikeChat(name, tooltip);

    tooltip:Show();
end

function HironCraftScan_FoundCrafterListElementMixin:OnEnter()
    if not self.info then return; end
    self.HoverBackground:Show();
    self:ShowTooltip();
end

function HironCraftScan_FoundCrafterListElementMixin:OnLeave()
    if not self.info then return; end
    self.HoverBackground:Hide();
    tooltip:Hide();
end

HironCraftScan_ItsMeIconMixin = {}
...
function HironCraftScan_ItsMeIconMixin:OnLeave()
    self:GetParent().HoverBackground:Hide();
    GameTooltip:Hide()
end

HironCraftScan_BusyIconMixin = {}

function HironCraftScan_BusyIconMixin:OnEnter()
    self:GetParent().HoverBackground:Show();
    GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
...
function HironCraftScan_BusyIconMixin:OnLeave()
    self:GetParent().HoverBackground:Hide();
    GameTooltip:Hide()
end
