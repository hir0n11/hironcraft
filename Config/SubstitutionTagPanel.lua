...
function HironCraftScan.Config.GetSuggestions(text, includeContext)
    local result = {}
    text = text:lower()
    local tags = HironCraftScan.Utils.saved(HironCraftScan.DB.settings, 'substitution_tags', {})
...
function HironCraftScan.Config.SubstitutionTagExists(key)
...
function HironCraftScan.Config.ContainsSubstitutionTags(text)
    return text:find('{')
end

function HironCraftScan.Config.AppendSubstitutionTagTooltip(text, tooltip, asTitle)
    local tags = HironCraftScan.Utils.saved(HironCraftScan.DB.settings, 'substitution_tags', {})
    local contains = {}
...
function HironCraftScan.Config.PopulateSubstitutionTagTooltip(text, reporter)
    local tags = HironCraftScan.Utils.saved(HironCraftScan.DB.settings, 'substitution_tags', {})
    for key in text:gmatch('{(.-)}') do
        if tags[key] then
            reporter:Info(function(tooltip)
                HironCraftScan.Config.AppendSubstitutionTagTooltip(text, tooltip, true)
            end)
            return
        end
    end
...
local LoadDynamicDefaults = function()
    ...
    for _, id in ipairs(professionIDs) do
        if HironCraftScan.Utils.IsCurrentExpansion(id) then
            ...
            if info.parentProfessionID then
                defaults[info.parentProfessionName] =
                    L(HironCraftScan.CONST.PROFESSION_DEFAULT_KEYWORDS[info.parentProfessionID])
            end
        end
    end
...
function HironCraftScan.Config.AutoCreateFromDefault(text)
    LoadDynamicDefaults()
...
function HironCraftScan.Config.SubstituteTags(text)
    local tags = HironCraftScan.DB.settings.substitution_tags or {}
    return HironCraftScan.Utils.FString(text, tags)
end

local panel = nil
function HironCraftScan.Config.LoadSubstitutionTagConfigOptions(configInfo)
    if not panel then
        panel = CreateFrame(
            'Frame',
            'HironCraftScanTagConfigPanel',
            HironCraftScanConfigPage.Options,
            'HironCraftScanTagConfigPanelTemplate'
        )
    end
    panel:Init()
    return panel
...
HironCraftScanTagConfigPanelMixin = {}

function HironCraftScanTagConfigPanelMixin:OnLoad()
    local padLeft = 0
    local pad = 5
    local spacing = 1
    ...
    view:SetElementFactory(function(factory, node)
        ...
        factory('HironCraftScanExistingTagEntryTemplate', Initializer)
    end)
...
function HironCraftScanTagConfigPanelMixin:Init()
    self.tags = HironCraftScan.Utils.saved(HironCraftScan.DB.settings, 'substitution_tags', {})
...
function HironCraftScanTagConfigPanelMixin:NewTagInvalid(value)
...
function HironCraftScanTagConfigPanelMixin:SetupNewTagInputs()
    self.defaults = {
        ['tag.autocomplete_enabled'] = true,
    }
    HironCraftScan.SetupTextInput(self, self.AddTag, 'tag.new')
    HironCraftScan.SetupCheckBox(self, self.AutoCompleteEnabled, 'tag.autocomplete_enabled')
    ...
    HironCraftScan.SetupButton(self.AddTagButton, 'tag.new.button', function()
    ...
    HironCraftScan.SetupInfoIcon(self.HeaderInfo, 'tag.header')
end

function HironCraftScanTagConfigPanelMixin:Validate(keyword, value, reporter)
...
function HironCraftScanTagConfigPanelMixin:GetConfigValue(keyword)
    if keyword == 'tag.new' then
        return self.new_tag or ''
    end

    return HironCraftScan.DB.settings[keyword] or self.defaults[keyword]
end

function HironCraftScanTagConfigPanelMixin:UpdateConfigValue(keyword, value)
    if keyword == 'tag.new' then
...
function HironCraftScanTagConfigPanelMixin:OnConfigChange()
    HironCraftScan.Config.OnConfigChange()
end

HironCraftScanExistingTagEntryMixin = {}

function HironCraftScanExistingTagEntryMixin:Init(node)
    self.tags = HironCraftScan.Utils.saved(HironCraftScan.DB.settings, 'substitution_tags', {})
    self.key = node.key
    self.KeyRegion.Key:SetText('{' .. node.key .. '}')
    HironCraftScan.SetupTextInput(self, self.Value, 'tag_value')
    self.Value.Expression.EditBox:SetScript('OnTabPressed', function(self)
        self:ClearFocus()
    end)

    HironCraftScan.Utils.SetupParentChildScrolling(self.Value.Expression, panel.ScrollBox)
...
function HironCraftScanExistingTagEntryMixin:GetConfigValue(keyword)
    return self.tags[self.key]
end

function HironCraftScanExistingTagEntryMixin:UpdateConfigValue(keyword, value)
    self.tags[self.key] = value
end

function HironCraftScanExistingTagEntryMixin:OnConfigChange()
    HironCraftScan.Config.OnConfigChange()
end
