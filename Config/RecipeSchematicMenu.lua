...
HironCraftScan.IsDecor = IsDecor
...
local function SaveConcentrationData(skillLineID, profConfig)
    if C_ProfSpecs.SkillLineHasSpecialization(skillLineID) then
        ...
        local concentrationData = HironCraftScan.ConcentrationData(currencyID)
...
local function GetParentContext()
    ...
    local name = HironCraftScan.GetPlayerName(true)
    local updated = false

    -- Create the character entry if this is the first time we're seeing it.
    local db = HironCraftScan.DB.characters[name]
    if not db then
        db = {
            professions = {},
            parent_professions = {},
            sourceID = HironCraftScan.DB.settings.my_uuid,
        }
        HironCraftScan.DB.characters[name] = db
    end
    ...
    if not dbPP then
        dbPP = HironCraftScan.Utils.DeepCopy(HironCraftScan.CONST.DEFAULT_PPCONFIG)
...
HironCraftScan.Events:Register('CONCENTRATION_UPDATED', function()
...
local function ScanAllRecipes(OnScanComplete, forceScan)
    ...
    HironCraftScan.Events:Emit('PROFESSION_SCAN_BEGIN')
    ...
    local waitGroup = HironCraftScan.WaitGroup(function()
        ...
        if ctxt.updated then
            for id, config in pairs(professions) do
                HironCraftScan.Config.UpdateProfession(ctxt.name, id, config)
            end
        ...
        HironCraftScan.Events:Emit('PROFESSION_SCAN_COMPLETE')
        ...
        if item and not item:IsItemEmpty() then
            ...
            item:ContinueOnItemLoad(function()
                ...
                if bindType ~= 0 then
                        ...
                        if not recipeConfig then
                            ...
                            recipeConfig = {
                                scan_state = recipeInfo.learned
                                        and HironCraftScan.CONST.RECIPE_STATES.PENDING_REVIEW
                                    or HironCraftScan.CONST.RECIPE_STATES.UNLEARNED,
                            }
                            if isDecor then
                                ctxt.dbCurrentExp.recipes[id] = recipeConfig
                            else
                    ...
                    if
                        recipeInfo.learned
                        and recipeConfig.scan_state == HironCraftScan.CONST.RECIPE_STATES.UNLEARNED
                    then
    ...
    local perFrame = 5
    HironCraftScan.TimeSlice(C_TradeSkillUI.GetAllRecipeIDs(), perFrame, ProcessRecipe, OnFinish)
end
...
local function CreateMenuShownButton()
    ...
    HironCraftScan.Events:Register('BUTTON_HEIGHT', function(...)
        button:UpdateHeight(...)
    end)
    button:UpdateHeight()

    HironCraftScan.Events:Register('UPDATE_RECIPE_LABEL', function(...)
        button:UpdateRecipeLabel(...)
    end)

    HironCraftScan.Events:Register('CHARACTER_DISABLED', function(name)
        local ctxt = GetParentContext()
...
local function OpenRecipe()
    ...
    if profConfig.recipes[recipeInfo.recipeID] then
        HironCraftScan.Events:Emit('OPEN_RECIPE', ctxt.name, profID, recipeInfo.recipeID, attachButton)
    end
end

HironCraftScan_ScannerConfigButtonMixin = {}

function HironCraftScan_ScannerConfigButtonMixin:AttachedMenuLoading()
    self:SetEnabled(false)
    self.Spinner:SetShown(true)
end

function HironCraftScan_ScannerConfigButtonMixin:AttachedMenuLoadComplete()
...
function HironCraftScan_ScannerConfigButtonMixin:UpdateRecipeLabel(recipeID, ctxt)
    ...
    if recipeConfig then
        self.ScanningLabel:SetText(HironCraftScan.RecipeStateName(recipeConfig, profConfig))
    else
...
function HironCraftScan_ScannerConfigButtonMixin:OnClick(button)
    if self.character_disabled then
        ...
        if ctxt then
            ...
            local OnScanComplete = function()
                HironCraftScan.Events:Emit('CHARACTER_ENABLED', ctxt)
            end
    ...
    if not self.tethered then
        HironCraftScanConfigPage:SetMenuCollapsed(false)
    end
    HironCraftScan.Events:Emit('TETHER_CHANGED', self.tethered)
end
...
local function OnRecipeSelected()
    ...
    local ctxt = GetParentContext()
    if ctxt then
        HironCraftScan.DoOnce(CreateMenuShownButton)

        HironCraftScan.State.professionID = ctxt.ppInfo.professionID
        HironCraftScan.Frames.MainButton:UpdateIcon()
    end

    HironCraftScan.Events:Emit('RECIPE_SELECTED', ctxt)
end
