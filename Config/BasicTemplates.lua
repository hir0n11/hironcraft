...
function HironCraftScan.SetupButton(field, keyword, OnClick)
...
function HironCraftScan.SetupCheckBox(panel, field, keyword, size)
    local title = L('dialog.' .. keyword)
...
function HironCraftScan.SetupInfoIcon(info, keyword)
    info:SetAlpha(0.25)
    info:SetScript('OnEnter', function(self)
        ...
        HironCraftScan.GameTooltip_AddWhite(L(tooltip_keyword))
...
function HironCraftScan.SetupTextInput(panel, field, keyword)
    InputScrollFrame_OnLoad(field.Expression)

    -- InputScrollFrameTemplate does not reliably size dynamically-created
    -- edit boxes after their parent is laid out. Without an explicit width,
    -- the edit box behaves like a single long line: text disappears behind
    -- the right edge instead of wrapping inside the visible scroll frame.
    local editBox = field.Expression.EditBox
    ...
    UpdateMultilineGeometry()
    if not field.Expression.craftScanMultilineGeometryHooked then
        field.Expression.craftScanMultilineGeometryHooked = true
        field.Expression:HookScript('OnSizeChanged', UpdateMultilineGeometry)
    end
    if C_Timer and C_Timer.After then
        C_Timer.After(0, UpdateMultilineGeometry)
    end
    ...
    if not instructions then
        ...
        if HironCraftScan.LOCAL:Exists(instructions) then
    ...
    field.Expression.EditBox:SetScript('OnEditFocusGained', function(self)
        if HironCraftScan.Config.ContainsSubstitutionTags(panel:GetConfigValue(keyword)) then
    ...
    field.Expression.EditBox:SetScript('OnEditFocusLost', function(self)
        ...
        local function DoUpdate()
            ...
            if HironCraftScan.Config.ContainsSubstitutionTags(self:GetText()) then
    ...
    if field.Info then
        HironCraftScan.SetupInfoIcon(field.Info, keyword)
    end
    ...
    if HironCraftScan.DB.settings['tag.autocomplete_enabled'] ~= false then
        ...
        local function AutoCompleteTag(self, BaseFn, ...)
            ...
            local suggestions =
                HironCraftScan.Config.GetSuggestions(tagPrefix, includeContextInAutoComplete)
...
HironCraftScanTextInputValidationIconMixin = {}
...
function HironCraftScanTextInputValidationIconMixin:Error(error)
    self:Clear()
...
function HironCraftScanTextInputValidationIconMixin:Warning(warning)
    self:Clear()
...
function HironCraftScanTextInputValidationIconMixin:Info(info)
    self:Clear()
    self:SetAtlas('Garr_Building-AddFollowerPlus')
...
function HironCraftScanTextInputValidationIconMixin:Clear()
...
function HironCraftScanTextInputValidationIconMixin:OK()
    return self.error == nil
end

function HironCraftScanTextInputValidationIconMixin:OnEnter()
    GameTooltip:SetOwner(self, 'ANCHOR_RIGHT')
    if self.error then
...
function HironCraftScanTextInputValidationIconMixin:OnLeave()
    GameTooltip:Hide()
end

function HironCraftScan.SetupSlider(panel, field, keyword, min, max, steps)
    field.Title:SetText(L('dialog.' .. keyword))
    HironCraftScan.SetupInfoIcon(field.Info, keyword)
    max = max or 1000
    min = min or 0
