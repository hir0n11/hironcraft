...
local function CreateDropdown(category, variable, key, name, options, tooltip, onChange)
    local default = HironCraftScan.CONST.DEFAULT_SETTINGS[key]
...
local function SetupMultiSelectDropdown(dropdown, setting, options, width, initTooltip)
    local function Inserter(setting, rootDescription)
        ...
        local function IsSelected(value)
            local values = HironCraftScan.DB.settings[setting.variableKey]
            if values == nil then
                return false
            end
        ...
        local function SetSelected(value)
            local values = HironCraftScan.Utils.saved(HironCraftScan.DB.settings, setting.variableKey, {})
...
HironCraftScanSettingsMultiSelectDropDownMixin = {}

function HironCraftScanSettingsMultiSelectDropDownMixin:SetupDropdownMenu(
    button,
    setting,
    options,
    initTooltip
...
local function CreateMultiSelectDropdown(category, variable, key, name, options, tooltip)
    ...
    local initializer = Settings.CreateControlInitializer(
        'HironCraftScanSettingsMultiSelectDropDownTemplate',
        setting,
        options,
        tooltip
    )
...
HironCraftScan.Utils.onLoad(function()
    local category, layout = Settings.RegisterVerticalLayoutCategory(L('HironCraftScan'))
    ...
    do
        ...
        local function GetOptions()
            local selected = HironCraftScan.DB.settings['disabled_addon_whitelist']
            local found = {}
        ...
        CreateMultiSelectDropdown(
            category,
            'HIRONCRAFT_SCAN_ADDON_WHITELIST',
    ...
    do
        local function GetOptions()
            local container = Settings.CreateControlTextContainer()
            container:Add(HironCraftScan.CONST.LEFT, L('Left'))
            container:Add(HironCraftScan.CONST.RIGHT, L('Right'))
            return container:GetData()
        end

        CreateDropdown(
            category,
            'HIRONCRAFT_SCAN_BANNER_DIRECTION',
            'banner_direction',
    ...
    do
        ...
        CreateSlider(
            ...
            nil,
            function()
                HironCraftScan.Events:Emit('BUTTON_HEIGHT')
            end
        )
        --L(LID.CUSTOMER_TIMEOUT_TOOLTIP));
    end
    do
        local GetValue = function()
            return HironCraftScan.DB.settings.discoverable
        end
        local SetValue = function(value)
            HironCraftScan.DB.settings.discoverable = value
        end
    ...
    do
        local GetValue = function()
            return HironCraftScan.DB.settings.permissive_matching
        end
        local SetValue = function(value)
            HironCraftScan.DB.settings.permissive_matching = value
    ...
    do
        ...
        local SetValue = function(value)
            HironCraftScan.DB.settings.show_chat_orders_tab = value
    ...
    do
        local GetValue = function()
            return HironCraftScan.DB.settings.collapse_chat_context
        end
        local SetValue = function(value)
            HironCraftScan.DB.settings.collapse_chat_context = value
        end
...
HironCraftScan.Settings = {}

function HironCraftScan.Settings:Open()
